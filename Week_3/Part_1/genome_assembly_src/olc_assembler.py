# genome_assembly_src/olc_assembler.py - Improved version with redundancy reduction
...
class OLCAssembler:
    ...
    def _is_contained(self, seq1, seq2, similarity_threshold=0.95):
        """Check if seq1 is contained within seq2 with some tolerance."""
        # If seq1 is much longer than seq2, it can't be contained
        if len(seq1) > len(seq2) * 1.1:
            return False
            
        # For small sequences, try exact containment
        if len(seq1) < 200:
            return seq1 in seq2
            
        # For longer sequences, check if a significant portion is contained
        found = False
        for i in range(0, len(seq2) - len(seq1) + 1, max(1, len(seq1) // 10)):
            # Calculate similarity over a window
            matches = sum(1 for j in range(min(len(seq1), len(seq2) - i)) 
                         if seq1[j] == seq2[i + j])
            similarity = matches / min(len(seq1), len(seq2) - i)
    ...
    def _calculate_similarity(self, seq1, seq2):
        ...
        # Use dynamic programming or other algorithm for similarity
        # Here we'll use a simple k-mer based approach for speed
        k = 10  # k-mer size
    ...
    def _cluster_similar_contigs(self, contigs, similarity_threshold=0.85):
        """Group similar contigs together."""
        # Only cluster if there are many contigs
        if len(contigs) <= 10:
            return contigs
            
        print("Clustering similar contigs...")
        clusters = []
        unclustered = list(contigs)  # Copy to avoid modifying during iteration
        
        # Start with the longest contig
        unclustered.sort(key=len, reverse=True)
        
        while unclustered:
            # Take the longest remaining contig as a new cluster center
            center = unclustered.pop(0)
            current_cluster = [center]
            
            # Find all similar contigs
            i = 0
            while i < len(unclustered):
                contig = unclustered[i]
                
                # If similar to the center, add to cluster
                similarity = self._calculate_similarity(center, contig)
                if similarity >= similarity_threshold:
                    current_cluster.append(contig)
                    unclustered.pop(i)
                else:
                    i += 1
        ...
        # Take the longest contig from each cluster
        print(f"Reduced {len(contigs)} contigs to {len(clusters)} clusters")
        return [max(cluster, key=len) for cluster in clusters]
    
    def _remove_contained_contigs(self, contigs):
        """Remove contigs that are contained within others."""
        if len(contigs) <= 1:
            return contigs
            
        print("Removing contained contigs...")
        
        # Sort by length (descending)
        sorted_contigs = sorted(contigs, key=len, reverse=True)
        filtered_contigs = []
        
        # For each contig, check if it's not contained in any larger contig
        for i, contig in enumerate(sorted_contigs):
            is_contained = False
            
            # Only check against larger contigs (which come first in the sorted list)
            for j in range(i):
                if self._is_contained(contig, sorted_contigs[j]):
                    is_contained = True
                    break
        ...
        print(f"Removed {len(contigs) - len(filtered_contigs)} contained contigs")
        return filtered_contigs
    
    def _reduce_redundancy(self, contigs):
        """Apply multiple redundancy reduction techniques."""
        original_count = len(contigs)
        
        # First remove contained contigs
        contigs = self._remove_contained_contigs(contigs)
        
        # Then cluster similar contigs
        contigs = self._cluster_similar_contigs(contigs)
        
        print(f"Reduced contigs from {original_count} to {len(contigs)} ({(len(contigs)/original_count*100):.1f}%)")
        return contigs
    ...
    def assemble(self, fastq_file, output_dir="./"):
        ...
        if len(contigs) > 10:  # Only apply to larger contig sets
            contigs = self._reduce_redundancy(contigs)
